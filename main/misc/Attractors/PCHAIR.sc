	...
	AND iTerminateAllAmbience = 0
		IF IS_CHAR_USING_MAP_ATTRACTOR this_ped
			...
			OR HAS_CHAR_BEEN_DAMAGED_BY_CHAR this_ped scplayer 
				iSetPizzaPanic = 1
				GOTO PCHAIR_PizzaPanic
			ELSE
				GOSUB PCHAIR_State_Machine
			ENDIF
		ELSE
			GOSUB PCHAIR_CleanUpBrain
		ENDIF		
	ELSE
		GOSUB PCHAIR_CleanUpBrain	
	ENDIF
...
PCHAIR_PedState1:  
	SWITCH iSubStateStatus	 				
   	...
   	CASE 0   	      		
   		TASK_PLAY_ANIM_NON_INTERRUPTABLE this_ped FF_Sit_In FOOD 4.0 FALSE FALSE FALSE TRUE -1
	...
	CASE 1
		fAnim_Time = 0.0
		IF IS_CHAR_PLAYING_ANIM this_ped FF_Sit_In
			GET_CHAR_ANIM_CURRENT_TIME this_ped	FF_Sit_In fAnim_Time			
		ENDIF
...
PCHAIR_PedState2: 
	SWITCH iSubStateStatus				
		...
		CASE 1	
			...
			IF $shop_name = &FDBURG
				SET_CHAR_USES_COLLISION_CLOSEST_OBJECT_OF_TYPE foriginalX foriginalY foriginalZ 1.5 CJ_BURG_CHAIR FALSE this_ped
			ELSE
...
PCHAIR_PedState3: 
	SWITCH iSubStateStatus				
		...
		CASE 0			
			...
			ENDIF
			TASK_PLAY_ANIM_NON_INTERRUPTABLE this_ped FF_Sit_out_180 FOOD 99.0 FALSE FALSE FALSE FALSE -1				
			++iSubStateStatus
		BREAK 
		...
		CASE 2
			fAnim_Time = 0.0
			IF IS_CHAR_PLAYING_ANIM this_ped FF_Sit_out_180
				GET_CHAR_ANIM_CURRENT_TIME this_ped	FF_Sit_out_180 fAnim_Time
			   	IF fAnim_Time = 1.0					
					IF $shop_name = &FDBURG
						SET_CHAR_USES_COLLISION_CLOSEST_OBJECT_OF_TYPE foriginalX foriginalY foriginalZ 1.5 CJ_BURG_CHAIR TRUE this_ped
					ELSE
