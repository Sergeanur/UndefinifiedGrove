...
CONST_INT CAR_KATIE_UNLOCKED_HELP				29
...
//--- NURSE GIRL (KATIE)
	iGFSelfRespect[KATIE] = 50 
	iGFLikesPlayer[KATIE] = 0 	
	iGFDesiredSexAppeal[KATIE] = 50
	iGFRespectGiven[KATIE] = 300

	iGFLikesOnDate[KATIE] = 0
		SET_BIT iGFLikesOnDate[KATIE] 	LIKES_DINERS
		SET_BIT iGFLikesOnDate[KATIE] LIKES_TO_CRUISE
		SET_BIT iGFLikesOnDate[KATIE] LIKES_ENTERTAINMENT_ZONES
		SET_BIT iGFLikesOnDate[KATIE] LIKES_SHOPPING_ZONES
		SET_BIT iGFLikesOnDate[KATIE] LIKES_PARK_BEACH_ZONES
		SET_BIT iGFLikesOnDate[KATIE] LIKES_TO_CAUSE_ACCIDENTS_KILL_PEDS

	iGFLikesPlayerTraits[KATIE] = 0
		SET_BIT iGFLikesPlayerTraits[KATIE] FIT

	iGFDiaryOfBusyHours[KATIE] = 0		
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_2AM
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_4AM
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_6AM
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_8AM 
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_10AM
		SET_BIT iGFDiaryOfBusyHours[KATIE] H_MIDNIGHT
...
CREATE_OBJECT_NO_OFFSET Gen_doorEXT07 -2574.495 1153.023 54.669 iGFdoor[KATIE]
SET_OBJECT_HEADING iGFdoor[KATIE] -19.444
DONT_REMOVE_OBJECT iGFdoor[KATIE]
SET_OBJECT_COLLISION_DAMAGE_EFFECT iGFdoor[KATIE] FALSE
SET_OBJECT_PROOFS iGFdoor[KATIE] TRUE TRUE TRUE TRUE TRUE
	...
	IF IS_BIT_SET iActiveGF iTemp
		...
		SWITCH iGFidx		
			...
			CASE KATIE
				...
				CLEAR_BIT iActiveGF KATIE_BONUS_ACTIVE
			BREAK
		ENDSWITCH
	ENDIF
RETURN	
...
GF_Dating_Agent_PrintUnlockedCarInfo:
	...
	SWITCH iGFidx
		...
		BREAK
		CASE KATIE
			IF NOT IS_BIT_SET iActiveGF CAR_KATIE_UNLOCKED_HELP
				PRINT_HELP GF_H011 // message to inform player he now has the keys to her car 
				SET_BIT iActiveGF CAR_KATIE_UNLOCKED_HELP
			ENDIF 
		BREAK
		CASE MILLIE
...
GF_Dating_Agent_SynchStats:
	SWITCH iGFidx
		...
		CASE KATIE
			...
			IF iTempStat < iGFLikesPlayer[KATIE]
				IF iGFLikesPlayer[KATIE] > 100
					iGFLikesPlayer[KATIE] = 100
				ENDIF 
				IF iGFLikesPlayer[KATIE] < 0
					SET_INT_STAT GIRLFRIEND_KATIE 0
				ELSE
					iTempStat -= iGFLikesPlayer[KATIE]
					ABS iTempStat 
...
GF_Dating_Agent_Console_Commands:
	...
	IF NOT IS_STRING_EMPTY $GF_txtConsoleIn 
		...
		IF $GF_txtConsoleCheck = $GF_txtConsoleIn
			...
			iGFLikesPlayer[KATIE] = iGFSelfRespect[KATIE]
		...
		IF $GF_txtConsoleCheck = $GF_txtConsoleIn
			...
			iGFLikesPlayer[KATIE] = GF_LIKES_PLAYER_STAGE2
