...
GF_Sex_State2: 
	...
	CASE 1
		IF NOT GET_FADING_STATUS
			...
			ENDIF
			//--- Restore player collisions if sex was uncencored
			IF iCensoredVersion = 0 
	...
	CASE 5
		...
		GOSUB GF_Sex_SpankAtGivenSpeed // Reads: fTemp[0], returns end of anim by setting iSexMachineFlags 5  		
 		IF IS_BIT_SET iSexMachineFlags 5
			...
			REPORT_MISSION_AUDIO_EVENT_AT_CHAR scplayer SOUND_SPANK  
...
GF_Sex_State9: 
	...
	CASE 4
		IF TIMERB > 3000			
			...
			IF iSpanking > 0
				iCJSayContext = CONTEXT_GLOBAL_SPANKING_MUFFLED 
			ELSE
